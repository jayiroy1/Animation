...
public:
	...
	const int pnum_ = 8;
	float t1=0.3,t2=0.7;
	vmath::vec3 points_[8] = { { 0.0, 1.5, -2.0 }, { -3.0, 4.0, 2.3 }, { -6.0,
			1.5, -2.5 }, { -4.0, -1.5, 2.8 }, { 1.0, -5.0, -4.0 }, { 4.0, -1.0,
			3.0 }, { 7.0, 1.0, -2.0 }, { 3.0, 3.0, 3.7 } };
	vmath::vec4 quaternions_[8] = {
...
double anglebetween(quaternion p, quaternion q){
	double angle,  angle1;
	double dot1=q.x*p.x+q.y*p.y+q.z*p.z;
	double dot2=-1*q.x*p.x-q.y*p.y-q.z*p.z;
	angle =  acos((dot1));
		angle1= acos((dot2));
		if(angle<angle1) {
...
quaternion resultant(quaternion p, quaternion q, float u, double angle)
{
	quaternion result;
	result.s=(sin((1-u)*angle)/sin(angle))*p.s+(sin((u)*angle)/sin(angle))*q.s;
	result.x=(sin((1-u)*angle)/sin(angle))*p.x+(sin((u)*angle)/sin(angle))*q.x;
	result.y=(sin((1-u)*angle)/sin(angle))*p.y+(sin((u)*angle)/sin(angle))*q.y;
	result.z=(sin((1-u)*angle)/sin(angle))*p.z+(sin((u)*angle)/sin(angle))*q.z;

return result;
}

float (*quaterniontomatrix(quaternion q))[3]{
	float rMatrix[3][3];
	if(g_scene_.easeinEaseout && g_scene_.shuttleRotation){
	rMatrix[0][0]=(1-2*q.y*q.y-2*q.z*q.z);
	rMatrix[0][1]=(2*q.x*q.y-2*q.s*q.z);
	rMatrix[0][2]=(2*q.x*q.z+2*q.s*q.y);
	rMatrix[1][0]=(2*q.x*q.y+2*q.s*q.z);
	rMatrix[1][1]=(1-2*q.x*q.x-2*q.z*q.z);
	rMatrix[1][2]=(2*q.y*q.z-2*q.s*q.x);
	rMatrix[2][0]=(2*q.x*q.z-2*q.s*q.y);
	rMatrix[2][1]=(2*q.y*q.z+2*q.s*q.x);
	rMatrix[0][0]=(1-2*q.x*q.x-2*q.y*q.y);
	}
	else{
		rMatrix[0][0]=(1);
			rMatrix[0][1]=(0);
...
point rotate(quaternion q , point p){
	...
	point r;
	r.x= rMatrix[0][0]*p.x+rMatrix[0][1]*p.y+rMatrix[0][2]*p.z;
	r.y= rMatrix[1][0]*p.x+rMatrix[1][1]*p.y+rMatrix[1][2]*p.z;
	r.z= rMatrix[2][0]*p.x+rMatrix[2][1]*p.y+rMatrix[2][2]*p.z;
	return r;
}
...
point u2p(float k, float t){
	...
	point p1(
							...
							g_scene_.animate_.points_[(i-1) % 8][2]);
			point p2(g_scene_.animate_.points_[i][0],
					g_scene_.animate_.points_[i][1],
					g_scene_.animate_.points_[i][2]), p3(
					g_scene_.animate_.points_[(i + 1) % 8][0],
					g_scene_.animate_.points_[(i + 1) % 8][1],
					g_scene_.animate_.points_[(i + 1) % 8][2]), p4(
					g_scene_.animate_.points_[(i + 2) % 8][0],
					g_scene_.animate_.points_[(i + 2) % 8][1],
					g_scene_.animate_.points_[(i + 2) % 8][2]);
...
float u[2];
	...
	quaternion q(g_scene_.animate_.quaternions_[section][0],g_scene_.animate_.quaternions_[section][1],g_scene_.animate_.quaternions_[section][2],g_scene_.animate_.quaternions_[section][3]);
	quaternion p(g_scene_.animate_.quaternions_[(section+1)%8][0],g_scene_.animate_.quaternions_[(section+1)%8][1],g_scene_.animate_.quaternions_[(section+1)%8][2],g_scene_.animate_.quaternions_[(section+1)%8][3]);
	quaternion resultQuaternion = resultant(q,p,u[1],anglebetween(q,p));
			...
			glScalef(gmodel_.modelscale_[0], gmodel_.modelscale_[1],
				...
				for (unsigned i = 0; i < gmodel_.triangles_.size(); ++i) {
					point ap51(gmodel_.normals_[i][0], gmodel_.normals_[i][1],
							gmodel_.normals_[i][2]);
								point rotated=rotate(resultQuaternion,ap51);
								glNormal3f(rotated.x,rotated.y,rotated.z);


					for (int j = 0; j < 3; ++j) {
						point ap5(gmodel_.vertices_[gmodel_.triangles_[i][j]][0],
								gmodel_.vertices_[gmodel_.triangles_[i][j]][1],
								gmodel_.vertices_[gmodel_.triangles_[i][j]][2]);
						point rotated=rotate(resultQuaternion,ap5);
...
glTranslatef(present.x,present.y,present.z);
	...
	for (int i = 0; i < 3; ++i) {
		...
		for (int j = 0; j < 6; ++j) {
			point normalq(gmodel_.axis_[i * 6 + j][0], gmodel_.axis_[i * 6 + j][1],gmodel_.axis_[i * 6 + j][2]);
			point rotated=rotate(resultQuaternion,normalq);
		glVertex3f(rotated.x,rotated.y,rotated.z);
		}
	}
	glEnd();
...
void init_gui() {
	...
	TwAddSeparator(operbar, "", "");
	TwAddVarRW(operbar, "EaseInEaseOut", TW_TYPE_BOOL8, &g_scene_.easeinEaseout, "");
	TwAddVarRW(operbar, "ShuttleRotation", TW_TYPE_BOOL8, &g_scene_.shuttleRotation, "");
