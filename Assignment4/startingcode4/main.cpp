...
public:
	...
	vmath::vec3 cubescale_ = vmath::vec3(0.1f,0.1f,0.1f);
...
// set modelviewmatrix
	...
	//draw model;
	glPushMatrix();
	glScalef(gmodel_.modelscale_[0], gmodel_.modelscale_[1], gmodel_.modelscale_[2]);
	glTranslatef(-gmodel_.modelcenter_[0], -gmodel_.modelcenter_[1], -gmodel_.modelcenter_[2]);
	setmaterial("model");
	glBegin(GL_TRIANGLES);
	for(unsigned i = 0; i < gmodel_.triangles_.size(); ++i) {
		glNormal3f(gmodel_.normals_[i][0], gmodel_.normals_[i][1], gmodel_.normals_[i][2]);
		for(int j = 0; j < 3; ++j){
			glVertex3f(gmodel_.vertices_[gmodel_.triangles_[i][j]][0], gmodel_.vertices_[gmodel_.triangles_[i][j]][1], gmodel_.vertices_[gmodel_.triangles_[i][j]][2]);
		}
	}
	glEnd();
	glPopMatrix();
	...
	for(int i = 0; i < 3; ++i){
		...
		for(int j = 0; j < 6; ++j) {
			glVertex3f(gmodel_.axis_[i * 6 + j][0], gmodel_.axis_[i * 6 + j][1], gmodel_.axis_[i * 6 + j][2]);
		}
	}
	glEnd();
	glPopMatrix();
